...
#  if defined(BOOST_PROCESS_WINDOWS_USE_NAMED_PIPE) 
        ...
        { 
            ...
            { 
                ::CloseHandle(overlapped.hEvent); 
                boost::throw_exception(boost::system::system_error(::GetLastError(), boost::system::system_category(), (const char *)"boost::process::detail::pipe::pipe: ConnectNamedPipe failed"));
            } 
        } 
        ::CloseHandle(overlapped.hEvent); 
#  else 
        if (!::CreatePipe(&hs[0], &hs[1], &sa, 0))
            boost::throw_exception(boost::system::system_error((int)::GetLastError(), boost::system::system_category(), "boost::process::detail::pipe::pipe: CreatePipe failed")); 
#  endif 
#endif 
